...
public class Main extends JPanel {
	...
	String file="Test_Resource_MyBooks_TB1_R1.xlsx";
	...
	{
		testMethods=obData.getData(resourcePath+file, testCase.get(1));
		return testMethods;
	}
	...
	public void test1(HashMap<Integer, ArrayList<String>> data2) throws Exception {
		...
		try {
			...
			for (i = 1; i < data2.size(); i++) {
				pbar2.setValue(i);
				try {
					msg2=this.setSize(data2.get(i).get(1));
					...
					System.out.println("no such method -"+mName+" "+msg+" "+msg2);
					continue;
				} catch (Exception e) {
					throw e;
				}
				if (!("closeBrowser").equals(mName)) {
					...
					report.write("file:///"+ssPath + name + " sucess " + i + ".png");
				}
			}
			report.createNewRow();
			report.write("");
			...
			report.write("");
			report.write(data2.get(i).get(1));
			report.write("file:///"+ssPath + name + " fail " + i + ".png");
